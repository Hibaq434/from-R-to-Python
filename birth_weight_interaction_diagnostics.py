...
fig.suptitle('Diagnostic Plots for Birth Weight ~ Age + Smoke*Race', y=1.02)
...
# Create a new figure for interaction plot
plt.figure(figsize=(10, 6))
for race in birthwt_noout['race'].unique():
    for smoke in ['Yes', 'No']:
        subset = birthwt_noout[(birthwt_noout['race'] == race) & (birthwt_noout['smoke'] == smoke)]
        if len(subset) > 0:
            plt.scatter(subset['age'], subset['bwt'], 
                       label=f'Race {race}, Smoke {smoke}', 
                       alpha=0.6)

plt.xlabel('Mother\'s Age')
plt.ylabel('Birth Weight (g)')
plt.title('Birth Weight by Age, Race, and Smoking Status')
plt.legend()
plt.show() 