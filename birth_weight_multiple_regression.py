...
X = pd.get_dummies(birthwt_noout[['age', 'smoke']], drop_first=True)
...
print("\nCall:")
print("lm(formula = bwt ~ smoke + age, data = birthwt_noout)")
...
print(f"smoke_Yes   {model.params[2]:10.2f} {model.bse[2]:10.2f} {model.tvalues[2]:8.2f} {model.pvalues[2]:8.4f} ***")
print("-" * 50)
...
print(f"F-statistic: {model.fvalue:.2f} on 2 and {model.df_resid} DF, p-value: {model.f_pvalue:.4f}")
